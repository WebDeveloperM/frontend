...
export default function ComputerTable({
    ...
    const extractPrefix = (s: string): number => {
        const m = s.match(/^(\d+)/);
    ...
    useEffect(() => {
        if (isFiltered) {
            ...
            // setComputers(checkedComputer.slice(page * rows, (page + 1) * rows));
    ...
    const [sortField, setSortField] = useState<string>('departament.name');
    const [sortOrder, setSortOrder] = useState<number>(1);

    const sortByDepartment = (data: Compyuter[]): Compyuter[] => {
        return [...data].sort((a, b) => {
            const n1 = extractPrefix(a.departament?.name);
            const n2 = extractPrefix(b.departament?.name);
            if (n1 === n2) {
                return a.departament.name
                    .localeCompare(b.departament.name, undefined, {sensitivity: 'base'});
            }
            return n1 - n2;
        });
    };
    const handleSort = (e) => {
        const {field, order, data} = e;
        ...
        if (field === 'departament.name') {
            data.sort((a, b) => {
                ...
                const n2 = extractPrefix(b.departament?.name);
                if (n1 === n2) {
                    return order * a.departament.name.localeCompare(b.departament.name, undefined, {sensitivity: 'base'});
                }
                return order * (n1 - n2);
            });
            setComputers([...data]);
        ...
        <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
            ...
            >
                ...
                <Column
                    ...
                    filterFunction={(_cellValue, _filter, rowData) => {
                        const raw = filters.history_date.value;
                        if (raw) {
                            const filterDate = raw instanceof Date ? raw : new Date(raw);
                            if (!rowData.history_date) {
                                return false;
                            }
                            const rowDate = new Date(rowData.history_date).toDateString();
                            if (rowDate !== filterDate.toDateString()) {
                                return false;
                            }
                        }

                        const userFilter = filters.history_user.value;
                        if (userFilter) {
                            const name = (rowData.history_user || '').toLowerCase();
                            if (!name.includes((userFilter as string).toLowerCase())) {
                                return false;
                            }
                        }
