...
export default function ComputerTable({
    checkedComputer,
    setDeleteCompForChecked, isFiltered
}: Props) {
    ...
    console.log(filterOptions, "5555555555555");

    useEffect(() => {
        if (isFiltered) {
            // Client-side pagination for filtered data
            const page = Math.floor(first / rows);
            const paginatedData = checkedComputer.slice(page * rows, (page + 1) * rows);
            setComputers(paginatedData);
            setTotalCount(checkedComputer.length);
        } else {
            const page = Math.floor(first / rows) + 1;
            axioss
                .get(`${BASE_URL}/all_compyuters/?page=${page}&page_size=${rows}`)
                .then((res) => {
    ...
    }, [first, rows, isFiltered, checkedComputer]);
    ...
    const fetchComputers = (url = `${BASE_URL}/all_compyuters/`) => {
        setLoading(true);
        axioss
            .get(url)
            .then((res) => {
                ...
                <div className="flex items-center gap-3">
                    ...
                    <InputText
                        ...
                        onChange={(e) => {
                            ...
                            fetchComputers(`${BASE_URL}/all_compyuters/?search=${e.target.value}`);
                        }}
                        placeholder="Поиск ..."
                    />
